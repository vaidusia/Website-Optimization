...
var resizePizzas = function(size) {
    ...
    function changePizzaSizes(size) {
        ...
        for (var i=0; i < randomPizzasLength; i++) {
            randomPizzas[i].style.width = newWidth + "%"; // change the pizzas width when we move the slider
        }
    }
...
function updatePositions() {
    ...
    // Make calculations of pizza's phases on the screen when the webpage is scrolled
    for (var i = 0; i < itemsLength; i++) {
        phase[i] = Math.sin(scrollTop + (i % 5));
    }
    
    // Moves the background pizzas
    for (var i = 0; i < itemsLength; i++) {
        items[i].style.transform = 'translateX(' + (items[i].basicLeft + 100 * phase[i] - 1024 + 'px') + ')';
    }
    
    // User Timing API to the rescue.
